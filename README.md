...
Cluster algebras were introduced by Fomin and Zelevinsky in order to give an algebraic framework for total positivity and canonical bases in Lie theory. One starts with a **seed**: a cluster $(x_1, \ldots, x_n)$ of rational functions together with a skew-symmetrizable integer matrix $B$. **Mutation** at index $k$ replaces $x_k$ by a new rational function via
...
Cluster algebras arise naturally in total positivity, the representation theory of quivers, coordinate rings of Grassmannians and flag varieties, and Conway–Coxeter frieze patterns.  They have deep connections to mathematical physics, including the thermodynamic Bethe ansatz of 2D integrable field theories, BPS state counting in supersymmetric gauge theories, and the symbol alphabet of scattering amplitudes in $\mathcal{N}=4$ SYM.  See the notebook on [Y Systems and the Thermodynamic Bethe Ansatz](examples/y_systems_and_tba.ipynb) for worked examples.
